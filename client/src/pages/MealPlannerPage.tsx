// client/src/pages/MealPlannerPage.tsx
...
const MealPlannerPage: React.FC = () => {
  ...
  const addRecipeToPlan = (recipe: IFrontendRecipe, date: string, meal: string) => {
    setMealPlan(prevPlan => ({
      ...
      [date]: {
        ...(prevPlan[date] || {}),
        [meal]: { recipeId: recipe._id, recipeTitle: recipe.title },
      },
    }));
  };
  ...
  const handleSavePlan = async () => {
    ...
    setError(null);
    try {
      await saveMealPlan(mealPlan, authToken);
      setIsEditing(false);
    } catch (err) {
    ...
    <div className="min-h-screen px-4 py-8 font-[Poppins] bg-neutral-50 text-gray-800">
      <div className="mx-auto xl:px-8">
          ...
          <div className="flex items-center space-x-4">
            {!isEditing && (
              <button 
                ...
                className="py-2 px-6 rounded-full text-white font-semibold transition duration-300 bg-gray-500 hover:bg-gray-600"
              >
                Edit Plan
              </button>
            )}
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="lg:w-1/4 bg-white p-6 rounded-lg shadow-xl h-fit sticky top-8">
            <div className="mb-4">
              <input
                type="text"
            ...
            {filteredRecipes.length > 0 ? (
              // Change: Increased max-height to show more recipes
              <ul className="space-y-2 overflow-y-auto max-h-[32rem] pr-2">
                {filteredRecipes.map(recipe => (
                  <li 
                    key={recipe._id} 
                    className={`p-3 rounded-md cursor-pointer transition duration-150 ${selectedRecipeId === recipe._id ? 'bg-orange-100 border-2 border-orange-500' : 'bg-gray-100 hover:bg-gray-200'}`}
          ...
          <div className="grid grid-cols-[1fr_repeat(7,minmax(0,1fr))] gap-4 text-center w-full">
            <div className="p-4 bg-gray-100 rounded-lg font-semibold text-center flex items-center justify-center">Meal</div>
            
            {days.map((day, index) => (
              <div 
                key={day} 
                className={`p-4 rounded-lg font-semibold ${index === currentDayIndex ? 'bg-orange-100' : 'bg-gray-100'}`}
              >
                <div>{day}</div>
            ...
            {meals.map(meal => (
              <React.Fragment key={meal}>
                {/* Change: Added h-24 to match the meal slot height */}
                <div className="p-4 bg-gray-100 rounded-lg font-semibold flex items-center justify-center h-24">{meal}</div>
                
                {weekDates.map((date, index) => {
                  const dateKey = date.toISOString().split('T')[0];
                  const entry = mealPlan[dateKey]?.[meal];
                  
                  const mealSlotClasses = `p-2 rounded-lg h-24 flex flex-col items-center justify-center transition duration-200 ${
                    entry ? 'border-2 border-solid border-orange-500' : 'border-2 border-dashed border-gray-300'
                  } ${isEditing ? 'cursor-pointer' : ''} ${index === currentDayIndex ? 'bg-orange-50' : ''}`;
                    ...
                    <div
                      ...
                      onClick={() => {
                        if (isEditing && selectedRecipeId) {
                          const recipeToAdd = recipes.find(r => r._id === selectedRecipeId);
                          if (recipeToAdd) {
